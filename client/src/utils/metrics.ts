...
export function calculateFinancialHealthScore(data: FinancialData) {
  ...
  let scores = {
    ...
    debtToIncomeScore: Math.max(
      0,
      maxScores.debtToIncomeRatio - debtToIncomeRatio * 10,
    ),
    expenseScore: Math.max(0, maxScores.expenseRatio - expenseRatio * 10),
    debtPaymentScore: Math.max(
      0,
      maxScores.debtPaymentRatio - debtPaymentRatio * 10,
    ),
    assetToDebtScore: Math.min(
      assetToDebtRatio * 10,
      maxScores.assetToDebtRatio,
    ),
  };
